...
model HeatingSystemMSLReduced "Simple model of a heating system"
...
public
  ...
  .Modelica.Thermal.HeatTransfer.Components.ThermalConductor wall(G=1.6e3/20)
    annotation (Placement(transformation(
        ...
        extent={{8,-10},{-8,10}},
        rotation=90)));
  replaceable .Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow burner(
    T_ref=343.15,
    alpha=-0.5)
    annotation (Placement(transformation(extent={{16.0,54.0},{36.0,74.0}},rotation = 0.0,origin = {0.0,0.0})));
  inner .Modelica.Fluid.System system(
      m_flow_small=1e-4, energyDynamics=.Modelica.Fluid.Types.Dynamics.SteadyStateInitial,massDynamics = Modelica.Fluid.Types.Dynamics.SteadyState)
                        annotation (Placement(transformation(extent={{-20.0,78.0},{0.0,98.0}},rotation = 0.0,origin = {0.0,0.0})));
...
tankLevel = tank.level;
  ...
  connect(burner.port, heater.heatPorts[1])
                                          annotation (Line(
      points={{36,64},{40.1,64},{40.1,24.4}}, color={191,0,0}));
